...
## Github link: https://github.com/ANTsX/ANTs/blob/v2.3.4/Scripts/antsCorticalThickness.sh
...
# JNL EDITS: try lowering the gradient step size to 0.0125 to account for smaller dog voxels (default 0.025) and smoothing parameter to 0.75 (default 1.5)
DIRECT=${ANTSPATH}/KellyKapowski
DIRECT_CONVERGENCE="[ 45,0.0,10 ]"
DIRECT_THICKNESS_PRIOR="10"
DIRECT_GRAD_STEP_SIZE="0.0125"
DIRECT_SMOOTHING_PARAMETER="0.75"
...
if [[ $DEBUG_MODE -gt 0 ]];
  then
    ...
    # costs about 10 minutes an iteration
...
## CELS Edits- just calling the modified script
    if [[ -f ${EXTRACTION_REGISTRATION_MASK} ]]
      then
        logCmd $code_path/antsBrainExtraction_k9.sh \
          -d ${DIMENSION} \
