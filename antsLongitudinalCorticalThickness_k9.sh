...
## JN EDIT: tested antsMultivariateTemplateConstruction.sh by changing from -g 0.25 to -g 0.1, 5 iterations at 200x100x70x30x3
# not helpful; changed back
SINGLE_SUBJECT_TEMPLATE=${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}T_template0.nii.gz
...
if [[ ! -f ${SINGLE_SUBJECT_TEMPLATE_CORTICAL_THICKNESS} ]];
  then
    if [[ $DO_REGISTRATION_TO_TEMPLATE -eq 0 ]];
      then
        logCmd $code_path/antsCorticalThickness_k9.sh \
          -d ${DIMENSION} -n 5 \
          -q ${RUN_FAST_ANTSCT_TO_GROUP_TEMPLATE} \
